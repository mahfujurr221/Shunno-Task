...
<div class="card">
    ...
    <form class="form_container" method="POST" action="{{ route('food-items.update', $foodItem->id) }}"
        enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <!-- Laravel requires PUT or PATCH method for updating records -->
            ...
            <div class="mt-3 row">
                ...
                <div class="mb-2 col-md-3">
                    <img id="previewImage" class="img-fluid"
                        src="{{ asset($foodItem->image ?? 'backend/img/placeholder.jpg') }}" alt="Image" height="100"
                        width="100">
                </div>
                ...
                <div class="mb-2 col-md-6">
                    ...
                    <textarea class="form-control"
                        name="description">{{ old('description', $foodItem->description) }}</textarea>
                </div>
                <div class="mb-2 col-md-6">
                    <label class="form-label">Serial Number<span class="text-danger">*</span></label>
                    <input type="number" class="form-control {{ $errors->has('serial') ? 'is-invalid' : '' }}"
                        name="serial" placeholder="Enter Serial Number" value="{{ old('serial', $foodItem->serial) }}"
                        required>
                ...
                <div class="mb-2 col-md-6">
                    ...
                    <select name="type" class="form-select {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        ...
                        <option value="new" {{ $foodItem->type == 'new' ? 'selected' : '' }}>New</option>
                        <option value="popular" {{ $foodItem->type == 'popular' ? 'selected' : '' }}>Popular</option>
                        <option value="special" {{ $foodItem->type == 'special' ? 'selected' : '' }}>Special</option>
                    </select>
                    @if($errors->has('type'))
                ...
                <div class="mb-2 col-md-6">
                    ...
                    <select name="status" class="form-select {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <option value="1" {{ $foodItem->status == 1 ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $foodItem->status == 0 ? 'selected' : '' }}>Inactive</option>
                    </select>
